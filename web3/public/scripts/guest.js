/**
 * Copyright 2018 Jong-Liang Nieh All Rights Reserved.
...
function askNewGame() {
  return firebase.firestore().collection('sessions').doc(sessionToken).set({
    ...
    console.error('Error creating new game to Firebase Database', error);
  });
}
...
function onMessageFormReset(e) {
  ...
  if (!checkSignedInWithMessage()) return;

  // to-do: save and close the game
  // to-do: ask a new game  

  askNewGame().then(function() {
    // Clear message text field and re-enable the SEND button.
    resetMaterialTextfield(messageInputElement);
    toggleButton();
  });
